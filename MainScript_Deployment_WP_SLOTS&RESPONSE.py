...
for index, node in enumerate(nodes):
    if node['type'] == "frame":
        temp_item = next(node for node in nodes if node['type'] == "frame")
        print("\nCurrent slot: " + temp_item['title'])
        slot_count = input(
            "How many options does this slot require (numeric)? ")  # here we begin to add details to the slot
        print("Please provide the slot variables now, press enter after each:")
        node_ids = random.sample(range(2000000000001, 3000000000000), int(slot_count))
        response_id = random.sample(range(3000000000001, 4000000000000), 1)
        response_previous_sibling = "node_19_1501070902849"  # hard coded for now --------------FIX FROM HERE
        temp_item = next(node for node in nodes if node['type'] == "frame")  # added after tp stpü stiüod errpr
        response_rubric = {"type": "response_condition",
                           "output": {
                               "text": {
                                   "values": [
                                       ""
                           ...
                           "parent": temp_item['title'],
                           "dialog_node": f"node_99_{response_id[0]}",
                           "previous_sibling": response_previous_sibling}  # needs to be changed to event handlers
        del response_rubric['previous_sibling']  # remove later
        nodes.append(response_rubric)
        for i in range(int(slot_count)):
            ...
            if i == 0:
                slot_rubric = {
                    ...
                    "metadata": {},
                    "variable": "$" + variable,
                    "dialog_node": f"slot_{countx}_{node_ids[i]}"}  # ,
                # "previous_sibling": f"{response_rubric['dialog_node']}"} FIX THIS AND RESPONSE ETC
                nodes.append(slot_rubric)
            else:
                slot_rubric = {
                    "type": "slot",
                    "output": {},
                    ...
                    "variable": "$" + variable,
                    "dialog_node": f"slot_{countx}_{node_ids[i]}",
                    "previous_sibling": f"slot_{countx - 1}_{node_ids[i - 1]}"}  # can countx-1 just be i?? same thing right
                nodes.append(slot_rubric)
